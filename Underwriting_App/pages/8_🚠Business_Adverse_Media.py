...
def clear_results_only():
    """Keep inputs, clear results."""
    for k in ["output_text", "sources"]:
        if k in st.session_state:
            del st.session_state[k]
...
if require_role(["Risk", "Underwriting"], "Exposure Decay Portfolio"):
    ...
    with col1:
        if st.button("Search"):
            if not (business_legal_name and business_dba_name and city_state):
    ...
    with col2:
        ...
        st.button("🧾 Clear results only", on_click=clear_results_only)
